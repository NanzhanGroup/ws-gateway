...
func TestParseTaskCommand_Hits(t *testing.T) {
	cases := []struct {
		...
		// 二次确认：##<编号> <指令>##
		{"##1000 删除##", 1000, "删除", true},
		{"##1000 rm##", 1000, "删除", true},
		{"##9999 删除##", 9999, "删除", true},
		// 「确认删除」别名：单井号也判为二次确认
		{"#1000 确认删除#", 1000, "删除", true},
		// 容错：空白 / 全角冒号逗号 / 井号冗余（### 及以上）
		{"  #1000   继续#  ", 1000, "继续", false}, // 首尾与中间冗余空白
		{"#1000：暂停#", 1000, "暂停", false},       // 全角冒号
		{"#1000，继续#", 1000, "继续", false},       // 全角逗号
		{"###1000 删除###", 1000, "删除", true},    // 井号冗余 → 仍判确认
		// 容错：手机中文键盘全角
		{"＃1000 继续＃", 1000, "继续", false},  // 全角井号
		{"＃1000　继续＃", 1000, "继续", false},  // 全角空格
		{"＃＃1000 删除＃＃", 1000, "删除", true}, // 全角双井号 → 确认
		{"＃１０００ 继续＃", 1000, "继续", false},  // 全角数字
	}
	for _, c := range cases {
...
// 关键：正常聊天内容绝不能被误吞。加了定界符后，**裸指令**（无 #）也应放行。
func TestParseTaskCommand_NoFalsePositive(t *testing.T) {
	cases := []string{
		"2024 年的合同还没签",
		"1000",
		"1000 继续",         // 裸指令已废弃 → 不拦截（无迁移提示，交给 LLM）
		"1000 删除",         // 同上：绝不因为"像个指令"就执行删除
		"#1000 继续",        // 只有前井号 → 不匹配
		"1000 继续#",        // 只有后井号 → 不匹配
		"#1000 继续吧兄弟#",    // 指令词后有内容 → 不整串匹配
		"请回复 #1000 继续# 哦", // 前后有字 → 不整串匹配
		"#100 继续#",        // 3 位 → 不匹配
		"#10000 继续#",      // 5 位 → 不匹配
		"#0999 继续#",       // 超出 1000-9999
		"#1000 你好#",       // 井号内不是指令词
		"#1000 删除哦#",      // 不是已知指令词（后缀干扰）
		"",
		"#标签 内容#", // 井号内非 4 位数字
		"2024-09-18 报告",
	}
...
func TestFormatTaskList_MentionsShortNoAndHint(t *testing.T) {
	...
	for _, want := range []string{"#1000", "#1001", "已暂停", "执行中", "3/12", "#1000 继续#", "##1000 删除##"} {
